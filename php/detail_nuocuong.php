...
if (isset($_GET['id'])) {
    $id = intval($_GET['id']);
    $sql = "SELECT * FROM NuocUong WHERE NuocUongID = $id";
...
<head>
    ...
    <title><?= htmlspecialchars($product['TenNuocUong'] ?? 'Chi tiết sản phẩm') ?></title> 
    ...
    <div class="container">
      ...
      <div class="collapse navbar-collapse justify-content-center" id="navbarNav">
        <ul class="navbar-nav fw-bold text-uppercase w-100 d-flex justify-content-evenly custom-nav">
          ...
          <li class="nav-item"><a class="nav-link active" href="all_nuocuong.php">Nước uống</a></li>
...
<div class="container mt-3">
  <nav aria-label="breadcrumb" style="background-color: #0d97d4; padding: 10px 15px; border-radius: 5px;">
    <ol class="breadcrumb mb-0 text-white">
      ...
      <li class="breadcrumb-item"><a href="all_nuocuong.php" class="text-white text-decoration-none">Nước uống</a></li>
      <li class="breadcrumb-item active text-white" aria-current="page"><?= htmlspecialchars($product['TenNuocUong'] ?? '') ?></li>
    </ol>
  </nav>
</div>
  ...
  <div class="row">
    ...
    <div class="col-md-6">
      <div class="text-center">
        <img src="../img/nuocuong/<?= htmlspecialchars($product['HinhAnh'] ?? 'default.png') ?>" 
    ...
    <div class="col-md-6">
        <h1 class="h3 mb-3"><?= htmlspecialchars($product['TenNuocUong']) ?></h1>
        <p class="h4 text-danger mb-4 fw-bold">
        ...
        <li class="mb-2"><strong>Thành phần:</strong> <?= htmlspecialchars($product['ThanhPhan']) ?></li>
        <li class="mb-2">
            <strong>Dung tích:</strong> 
            <?= htmlspecialchars($product['DungTich']) ?> 
            <?= strtolower($product['ChungLoai']) === 'bình' ? 'l' : 'ml' ?>
        </li>
        <li class="mb-2"><strong>Quy cách:</strong> <?= htmlspecialchars($product['QuyCach']) ?></li>
        <li class="mb-2"><strong>Chủng loại:</strong> <?= htmlspecialchars($product['ChungLoai']) ?></li>
      ...
      <div class="alert alert-info border-primary" style="border: 2px dashed #0d6efd;">
        ...
        <strong>Free Ship cho đơn hàng từ 03 bình/thùng</strong>
      </div>
    </div>

<!-- SIDEBAR ADS -->
<div class="position-fixed" style="right: 20px; top: 50%; transform: translateY(-50%); z-index: 1000; width: 200px;">

...
$result = $conn->query("SELECT * FROM Gao WHERE GaoID = $GaoID");
if ($result->num_rows == 0) {
    die("Không tìm thấy sản phẩm!");
}
$product = $result->fetch_assoc();

$TenGao = $product['TenGao'];
$ThuongHieu = $product['ThuongHieu'];
$LoaiGao = $product['LoaiGao'];
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($TenGao)) {
        ...
        if (empty($error)) {
            $stmt = $conn->prepare("UPDATE Gao SET TenGao = ?, ThuongHieu = ?, LoaiGao = ?, TrongLuong = ?, GiaThanh = ?, ThongTin = ?, HinhAnh = ? WHERE GaoID = ?");
            if (!$stmt) {
                $error = "Lỗi CSDL: " . $conn->error;
            } else {
                $stmt->bind_param("sssdsssi", $TenGao, $ThuongHieu, $LoaiGao, $TrongLuong, $GiaThanh, $ThongTin, $new_image, $GaoID);
                if ($stmt->execute()) {
...
<head>
    <meta charset="UTF-8">
    <title>Cập nhật sản phẩm gạo</title>
...
<div class="container">
    <div class="row justify-content-center">
        <div class="col-xl-5 col-lg-6 col-md-8 border rounded my-5 p-4 mx-3">
            ...
            <h3 class="text-center">Cập nhật sản phẩm gạo</h3>
            ...
            <form method="post" enctype="multipart/form-data">
                <div class="form-group">
                    ...
                    <input value="<?= htmlspecialchars($TenGao) ?>" name="TenGao" required class="form-control" type="text">
                </div>
                <div class="form-group">
                ...
                <div class="form-group">
                    ...
                    <input value="<?= htmlspecialchars($LoaiGao) ?>" name="LoaiGao" required class="form-control" type="text">
                </div>
                <div class="form-group">
                    <label for="TrongLuong">Trọng lượng (kg)</label>
                    <input value="<?= htmlspecialchars($TrongLuong) ?>" name="TrongLuong" required class="form-control" type="number" step="1">
                </div>
                <div class="form-group">
                ...
                <div class="form-group">
                    ...
                    <img src="../img/gao/<?= htmlspecialchars($HinhAnh) ?>" style="max-height: 120px;">
                </div>
                <div class="form-group">

...
if (isset($_GET['id'])) {
    $id = intval($_GET['id']);
    $sql = "SELECT * FROM Gao WHERE GaoID = $id";
...
<head>
    ...
    <title><?= htmlspecialchars($product['TenGao'] ?? 'Chi tiết Gas') ?></title> 
...
<div class="container mt-3">
  <nav aria-label="breadcrumb" style="background-color: #0d97d4; padding: 10px 15px; border-radius: 5px;">
    <ol class="breadcrumb mb-0 text-white">
      ...
      <li class="breadcrumb-item"><a href="all_gas.php" class="text-white text-decoration-none">Gạo</a></li>
      <li class="breadcrumb-item active text-white" aria-current="page"><?= htmlspecialchars($product['TenGao'] ?? '') ?></li>
    </ol>
  </nav>
</div>
...
<div class="container my-5">
  <div class="row">
    <div class="col-md-6 text-center">
      <img src="../img/gao/<?= htmlspecialchars($product['HinhAnh'] ?? 'default.png') ?>" 
           alt="<?= htmlspecialchars($product['TenGao'] ?? '') ?>" 
           class="img-fluid mb-3" style="max-height: 500px;">
    </div>

    <div class="col-md-6">
      <h1 class="h3 mb-3"><?= htmlspecialchars($product['TenGao']) ?></h1>
      ...
      <ul class="list-unstyled mb-4">
        ...
        <li class="mb-2"><strong>Loại gạo:</strong> <?= htmlspecialchars($product['LoaiGao']) ?> kg</li>
        <li class="mb-2"><strong>Trọng lượng:</strong> <?= htmlspecialchars($product['TrongLuong']) ?> kg</li>
      ...
      <div class="alert alert-info border-success" style="border: 2px dashed #28a745;">
        <i class="bi bi-truck text-success me-2"></i>
        <strong>Giao miễn phí trong nội thành với đơn hàng từ 10kg trở lên.</strong>
      </div>
    </div>
  </div>
</div>

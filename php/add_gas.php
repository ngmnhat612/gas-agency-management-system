...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($TenGas)) {
        ...
        if (!in_array($imageFileType, $allowedTypes)) {
            ...
            $stmt = $conn->prepare("INSERT INTO Gas (TenGas, ThuongHieu, TrongLuongVoBinh, GiaThanh, ThongTin, HinhAnh) VALUES (?, ?, ?, ?, ?, ?)");
            if (!$stmt) {
                $error = "Lỗi kết nối cơ sở dữ liệu: " . $conn->error;
            } else {
                $stmt->bind_param("ssdsss", $TenGas, $ThuongHieu, $TrongLuongVoBinh, $GiaThanh, $ThongTin, $imageName);
                if ($stmt->execute()) {
                    $success = "Sản phẩm đã được thêm thành công!";
                    $TenGas = $ThuongHieu = $ThongTin = '';
                    $TrongLuongVoBinh = $GiaThanh = '';
                } else {
...
<div class="container">
    <div class="row justify-content-center">
        <div class="col-xl-5 col-lg-6 col-md-8 border rounded my-5 p-4 mx-3">
            ...
            <h3 class="text-center text-secondary mb-4">Thêm sản phẩm gas</h3>
            ...
            <form method="post" enctype="multipart/form-data">
                <div class="form-group">
                    ...
                    <input type="text" class="form-control" id="TenGas" name="TenGas" required value="<?= htmlspecialchars($TenGas) ?>">
                </div>
                ...
                <div class="form-group">
                    <label for="TrongLuongVoBinh">Trọng lượng vỏ bình (kg)</label>
                    <input type="number" class="form-control" id="TrongLuongVoBinh" name="TrongLuongVoBinh" step="0.1" required value="<?= htmlspecialchars($TrongLuongVoBinh) ?>">
                </div>

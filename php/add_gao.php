...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($TenGao)) {
        ...
        if (!in_array($imageFileType, $allowedTypes)) {
            ...
            $stmt = $conn->prepare("INSERT INTO Gao (TenGao, ThuongHieu, LoaiGao, TrongLuong, GiaThanh, ThongTin, HinhAnh) VALUES (?, ?, ?, ?, ?, ?, ?)");
            if (!$stmt) {
                $error = "Lỗi kết nối cơ sở dữ liệu: " . $conn->error;
            } else {
                $stmt->bind_param("sssdsss", $TenGao, $ThuongHieu, $LoaiGao, $TrongLuong, $GiaThanh, $ThongTin, $imageName);
                if ($stmt->execute()) {
                    $success = "Sản phẩm đã được thêm thành công!";
                    $TenGao = $ThuongHieu = $LoaiGao = $ThongTin = '';
                    $TrongLuong = $GiaThanh = '';
                } else {
...
<div class="container">
    <div class="row justify-content-center">
        <div class="col-xl-5 col-lg-6 col-md-8 border rounded my-5 p-4 mx-3">
            ...
            <h3 class="text-center text-secondary mb-4">Thêm sản phẩm gạo</h3>
            ...
            <form method="post" enctype="multipart/form-data">
                <div class="form-group">
                    <label for="TenGao">Tên gạo</label>
                    <input type="text" class="form-control" id="TenGao" name="TenGao" required value="<?= htmlspecialchars($TenGao) ?>">
                </div>
                ...
                <div class="form-group">
                    <label for="LoaiGao">Loại gạo</label>
                    <input type="text" class="form-control" id="LoaiGao" name="LoaiGao" required value="<?= htmlspecialchars($LoaiGao) ?>">
                </div>

                <div class="form-group">
                    <label for="TrongLuong">Trọng lượng (kg)</label>
                    <input type="number" class="form-control" id="TrongLuong" name="TrongLuong" step="1" required value="<?= htmlspecialchars($TrongLuong) ?>">
                </div>

                <div class="form-group">
                    <label for="GiaThanh">Giá thành (VNĐ)</label>
                    <input type="text" class="form-control" id="GiaThanh" name="GiaThanh" required value="<?= htmlspecialchars($GiaThanh) ?>" placeholder="Liên hệ">
                </div>
